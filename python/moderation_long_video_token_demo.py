...
from moderation_sdk.moderation_long_video import moderation_long_video,get_moderation_input,get_moderation_output,get_moderation_requestbody
from moderation_sdk.utils import init_global_env

if __name__ == '__main__':
    init_global_env("cn-north-4")
    #
    # access long video moderation jobs,post data by token
    #
    user_name = '******'
    password = '******'
    ...
    request_body = get_moderation_requestbody(task_name, description,input_param, output_param, "porn,terrorism,politics",
                                                                                    "ad,politics,porn,abuse,contraband,flood")
    # call interface use the url
    result = moderation_long_video(token, project_id, request_body)
    print(result)
